...
export class TableReducer extends BaseReducer {
  ...
  private updateTableBlock(context: ReducerContext): BlockDiff | null {
    if (context.tableHeaders.length === 0) return null;

    if (context.currentBlock?.type === "table") {
      const block = context.currentBlock as TableBlock;
      block.headers = [...context.tableHeaders];
      block.alignments = [...context.tableAlignments];
      block.rows = context.tableRows.map((row) => [...row]);

      // Increment Version to trigger ArkTS UI refresh
    ...
    const block: TableBlock = {
      ...
      version: 0, // Initialize version to 0 for new blocks
      text: "",
    };
  ...
  private fallbackToParagraph(
    ...
    if (context.tableAlignments.length > 0) {
      fallbackText +=
        "|" + context.tableAlignments.map((a) => "---").join("|") + "|\n";
    }
  ...
  canContinueTable(char: string, context: ReducerContext): boolean {
    ...
    if (
      context.tableCellBuffer === "" &&
      context.tableCurrentRow.length === 0
    ) {
      if (char === "\n") return true;
